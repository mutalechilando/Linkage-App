...
describe('useTimeZoneConversion', () => {
  it('Hook returns a fromClientDate and fromServerDate function', () => {
    ...
    const wrapper = (_ref) => {
      ...
      return /*#__PURE__*/_react.default.createElement(_index.ConfigProvider, {
        config: config
      }, children);
    };
    ...
    const {
      result
    } = (0, _reactHooks.renderHook)(() => (0, _index.useTimeZoneConversion)(), {
      wrapper
    });
  ...
  it('returns fromServerDate that corrects for server time zone', () => {
    const systemInfo = { ...defaultSystemInfo,
  ...
  it('returns fromServerDate which accepts number, valid date string, or date object', () => {
    ...
    const serverDateFromNumber = result.current.fromServerDate(millisecondsAfterUTC);
  ...
  it('returns fromServerDate which returns current timestamp if no argument is passed', () => {
    ...
    jest.spyOn(global.Date, 'now').mockImplementation(() => new Date('2020-10-15T12:00:00.000Z').valueOf());
